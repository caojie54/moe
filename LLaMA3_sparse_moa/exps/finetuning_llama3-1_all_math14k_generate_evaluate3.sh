...
eff_batch_size=32
path="/home2/caojie"
output_dir="${path}/outputs/LLaMA3-1_smoe_structure/${dataset}/b${eff_batch_size}_epoch${epochs}_warme${warmup_epochs}_loralayers${lora_layers}_lorar${lora_rank}_lora${lora_targets}_alpha${lora_alpha}_maxthre${max_threshold}_boolweights${bool_weights}_adanoisy${adapter_noisy}_padalayers${p_adapter_layers}_padaptersize${p_adapter_size}_prompt_layers${prompt_layers}_prompt_len${prompt_len}_swi_x${swi_x}_blr${blr}_maxseq${max_seq_len}_flashatt2${flash_attention2}_bf16${bf16}_${tag}/"
