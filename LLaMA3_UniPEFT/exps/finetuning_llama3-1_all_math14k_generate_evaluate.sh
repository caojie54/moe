...
eff_batch_size=32
path="/home2/caojie"
output_dir="${path}/outputs/LLaMA3-1_moe_structure_unipeft/${dataset}/b${eff_batch_size}_gpu${num_devices}_bsg${batch_size_gpu}_e${epochs}_we1_loral${lora_layers}_r${lora_rank}_lora${lora_targets}_alpha${lora_alpha}_enum${expert_num}_hydra${hydra_moe}_padapter_layers${p_adapter_layers}_padaptersize${p_adapter_size}_padapterhydra${p_adapter_hydra}_prompt_l${prompt_layers}_prompt_len${prompt_len}_swi_x${swi_x}_blr${blr}_maxseq${max_seq_len}_flashatt2${flash_attention2}_bf16${bf16}_${tag}/"
