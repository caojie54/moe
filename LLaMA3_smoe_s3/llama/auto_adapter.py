...
class AUTOAdapterLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, type_weight: torch.Tensor, adapted_type_weights:Optional[torch.Tensor]):
        # type_weight, adapted_type_weights: [bsz, seqlen]
        ...
        selected_adapted_type_weights = adapted_type_weights[batch_idx] # [m]

        selected_x = x[batch_idx] # [m, dim] ,m tokens selected for this expert
        selected_results = self.expert(selected_x, selected_adapted_type_weights)
