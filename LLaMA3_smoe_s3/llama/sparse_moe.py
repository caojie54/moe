...
class AdaMoLE(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, adapted_weights: torch.Tensor):
        # input [m, dim], adapted_weights:[m]
        flattened_inputs = inputs

        logits = self.router(flattened_inputs) # [m, num_experts]
        if self.noisy:
            #Noise logits
        ...
        gate_logits = F.sigmoid(logits) # [m, num_experts]
        thresholds = F.sigmoid(self.threshold_fn(flattened_inputs)) * (1 - adapted_weights.unsqueeze(-1)) # 对此类型专家，adapted_weights 越大，阈值越小，可选的专家数量增多
