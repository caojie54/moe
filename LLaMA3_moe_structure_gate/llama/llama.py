...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs, w_lora=False, w_prompt=False):
        ...
        if self.w_prompt:
            ...
            self.prompt_outgate = torch.nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], type_weight:Optional[torch.Tensor], lora_gate:Optional[torch.Tensor]):
        ...
        if self.w_lora:
            if 'Q' in self.lora_targets:
                xq = xq + lora_gate * self.lora_Q(x, type_weight[:,:,type_idx])
                type_idx += 1
            if 'K' in self.lora_targets:
                xk = xk + lora_gate * self.lora_K(x, type_weight[:,:,type_idx])
                type_idx += 1
            if 'V' in self.lora_targets:
                xv = xv + lora_gate * self.lora_V(x, type_weight[:,:,type_idx])
                type_idx += 1
        ...
        if self.w_prompt:
            ...
            output = output + self.prompt_outgate * experts_output

        if self.w_lora and 'O' in self.lora_targets:
            return self.wo(output) + lora_gate * self.lora_O(output, type_weight[:,:,type_idx])
        else:
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, type_weight:Optional[torch.Tensor], lora_gate:Optional[torch.Tensor]):
        if self.w_lora:
            type_idx = 0
            if 'FFN_UP' in self.lora_targets:
                out = F.silu(self.w1(x)) * (self.w3(x) + lora_gate * self.lora_UP(x, type_weight[:,:,type_idx]))
                type_idx += 1
            else:
                out = F.silu(self.w1(x)) * self.w3(x)
            
            if 'FFN_DOWN' in self.lora_targets:
                out = self.w2(out) + lora_gate * self.lora_DOWN(out, type_weight[:,:,type_idx])
            else:
                out = self.w2(out)
            return out
        else:
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, w_lora=False, w_prompt=False, w_padapter=False):
        ...
        if self.w_padapter:
            ...
            self.p_adapter_gate = torch.nn.Parameter(torch.zeros(1))
        ...
        if w_lora:
            ...
            for x in lora_targets:
                ...
                if x in FFN_targets:
                    self.FFN_type += 1
            self.lora_gate = torch.nn.Parameter(torch.zeros(1))
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        type_idx = 0
        h = x + self.attention.forward(self.attention_norm(x), start_pos, freqs_cis, mask, type_weight=type_weights[:,:,type_idx:type_idx+self.attention_type], lora_gate=self.lora_gate)
        # out = h + self.feed_forward.forward(self.ffn_norm(h))
        type_idx += self.attention_type
        residual = h
        h = self.ffn_norm(h)
        out = self.feed_forward.forward(h, type_weight=type_weights[:,:,type_idx:type_idx+self.FFN_type], lora_gate=self.lora_gate)
        type_idx += self.FFN_type
        if self.w_padapter:
            adapter_states = self.p_adapter(h, type_weight=type_weights[:,:,type_idx])
            out = out + self.p_adapter_gate * adapter_states
        out = residual + out
