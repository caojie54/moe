...
def get_args_parser():
    ...
    # load adapters
    parser.add_argument('--lora_path', default='', type=str, help="")
    parser.add_argument('--p_adapter_path', default='', type=str, help="")
    parser.add_argument('--prompt_path', default='', type=str, help="")
    parser.add_argument('--router_only', type=str2bool, nargs='?', const=True, default=False, help='finetune router only')
    ...
    parser.add_argument("--val_data_path", default="", type=str, help="valid dataset  path")
...
def main(args):
    ...
    if args.lora_path:
        ...
        load_result = model.load_state_dict(lora_ckpt, strict=False)
        assert len(load_result.unexpected_keys) == 0, f"Unexpected keys: {load_result.unexpected_keys}"
    if args.p_adapter_path:
        padapter_ckpt = torch.load(args.p_adapter_path, map_location='cpu')
        padapter_result = model.load_state_dict(padapter_ckpt, strict=False)
        assert len(padapter_result.unexpected_keys) == 0, f"Unexpected keys: {padapter_result.unexpected_keys}"
    if args.prompt_path:
        prompt_ckpt = torch.load(args.prompt_path, map_location='cpu')
        prompt_result = model.load_state_dict(prompt_ckpt, strict=False)
        assert len(prompt_result.unexpected_keys) == 0, f"Unexpected keys: {prompt_result.unexpected_keys}"
    ...
    dataset_train = FinetuneDataset(args.data_path, llama_tokenzier_path,
                    max_tokens=args.max_seq_len, partition="train")
    print(dataset_train)
