...
def main(
    ...
    for batch in tqdm(batchs):
        ...
        results, time_cost = model.generate_time(prompts, max_gen_len=max_gen_len, temperature=temperature, top_p=top_p, time_gen=time_gen)
        
        # batch time_cost,
        if time_gen:
            bs = len(results)
            time_cost['batch_size'] = bs
    ...
    if time_gen:
        time_stas = {}
        all_cost = [cost['all_cost'] for cost in time_costs]
        inference_cost = [cost['inference_cost'] for cost in time_costs]
        total_ex = [cost['batch_size'] for cost in time_costs]
        time_stas['all_cost'] = sum(all_cost)
        time_stas['inference_cost'] = sum(inference_cost)
        time_stas['total_ex'] = sum(total_ex)
        time_stas['mean_all_cost'] = sum(all_cost) / sum(total_ex)
        time_stas['mean_inference_cost'] = sum(inference_cost) / sum(total_ex)

        time_save_path = save_path.replace('.jsonl', f'bs{max_batch_size}_timeC.jsonl')
        with open(time_save_path, 'a', encoding='utf-8') as f:
            json_data = json.dumps(time_stas, ensure_ascii=False)
            f.write(json_data + '\n')
