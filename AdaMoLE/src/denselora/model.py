...
class DenseLoraModel(LoraModel):
    """
    denselora
    ...
    def _create_and_replace(
        self, dense_lora_config: DenseLoraConfig, adapter_name: str,
        ...
        """
        kwargs = {
            "lora_rank": dense_lora_config.lora_rank,
            "lora_alpha": dense_lora_config.lora_alpha,
            "lora_dropout": dense_lora_config.lora_dropout,
            "init_lora_weights": dense_lora_config.init_lora_weights,
        }
        ...
        if not hasattr(self, "lora_share_q_A"):
            ...
            if self.model.config.architectures[0] == 'Qwen3ForCausalLM':
                ...
                if self.model.config.hidden_size == 4096:
                    ...
                    kv_out_dim = 1024
                    up_out_dim = 14336
            self.lora_share_q_A[adapter_name] = nn.Linear(dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_q_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, dim, bias=False)
            self.lora_share_k_A[adapter_name] = nn.Linear(dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_k_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, kv_out_dim, bias=False)
            self.lora_share_v_A[adapter_name] = nn.Linear(dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_v_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, kv_out_dim, bias=False)
            self.lora_share_o_A[adapter_name] = nn.Linear(dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_o_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, dim, bias=False)
            self.lora_share_up_A[adapter_name] = nn.Linear(dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_up_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, up_out_dim, bias=False)
            self.lora_share_down_A[adapter_name] = nn.Linear(up_out_dim, dense_lora_config.lora_rank, bias=False)
            self.lora_share_down_B[adapter_name] = nn.Linear(dense_lora_config.lora_rank, dim, bias=False)
        ...
        if isinstance(target, DenseLoraLayer):
    ...
    def _create_new_module(adapter_name: str, target: nn.Module, **kwargs: Any) -> nn.Module:
        ...
        if isinstance(target_base_layer, torch.nn.Linear):
            new_module = LinearDenseLoraLayer(base_layer=target, adapter_name=adapter_name, **kwargs)
        else:
            raise ValueError(
