...
class LLaMA_adapter(nn.Module):
    ...
    def forward_inference_router_stat(self, tokens, start_pos: int, layer_stat):
        ...
        for i,layer in enumerate(self.llama.layers):
            ...
            if start_pos == 0:
                layer_stat[i] = {}
                layer_stat[i]['sum_weights'] = torch.sum(layer.feed_forward.lora_DOWN.route_weight, 0)  # flattened
            else:
                layer_stat[i]['sum_weights'] +=  torch.sum(layer.feed_forward.lora_DOWN.route_weight, 0) 
    ...
    def generate_router_stat(
        ...
        # layer_stat['token_num'] = bsz * total_len  # batch1 有early stop
    ...
    def forward_inference_router_case(self, tokens, start_pos: int, layer_stat):
        ...
        for i,layer in enumerate(self.llama.layers):
            ...
            if start_pos == 0:
                layer_stat[i] = {}
                layer_stat[i]['weights'] =  layer.feed_forward.lora_DOWN.route_weight.clone() # flattened
            else:
                layer_stat[i]['weights'] = torch.cat((layer_stat[i]['weights'], layer.feed_forward.lora_DOWN.route_weight), dim=0)
    ...
    def generate_router_case(
        ...
        for i, t in enumerate(tokens.tolist()):
            ...
            try:
                ...
                print(len(words), layer_stat[0]['weights'].shape[1])
                print(words)
                print(total_len)
        return decoded, layer_stat